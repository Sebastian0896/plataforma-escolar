...
export function DashboardContent({ data }: DashboardContentProps) {
  ...
  const getAccesosRapidos = () => {
    ...
    if (rol === 'coordinador' && centroId) {
      accesos.push(
        { 
          ...
          icon: Users, 
          color: 'bg-cyan-500',
          description: 'Supervisar planificaciones'
        },
        { 
          href: '/dashboard', 
    ...
    if (rol === 'docente') {
      accesos.push(
        { href: '/admin/docente/diario', label: 'Diario del Docente', icon: CalendarDays, color: 'bg-blue-500', description: 'Registrar actividades' },
        { href: '/admin/docente/asistencia', label: 'Pase de Lista', icon: ClipboardCheck, color: 'bg-green-500', description: 'Marcar asistencia' },
        { href: '/admin/planificaciones/nueva', label: 'Nueva Planificación', icon: BookOpen, color: 'bg-purple-500', description: 'Crear planificación' }
      )
    }
    
    if (rol === 'admin_centro' && centroId) {
      accesos.push(
        { href: `/admin/centro/${centroId}`, label: 'Panel del Centro', icon: School, color: 'bg-cyan-500', description: 'Gestionar centro' },
        { href: '/admin/usuarios/centros', label: 'Gestión de Usuarios', icon: Users, color: 'bg-slate-500', description: 'Administrar usuarios' }
      )
    }
    
    if (rol === 'admin' || rol === 'superadmin') {
      accesos.push(
        { href: '/admin', label: 'Panel Administrativo', icon: TrendingUp, color: 'bg-primary', description: 'Gestión global' },
        { href: '/admin/centros', label: 'Gestión de Centros', icon: School, color: 'bg-cyan-500', description: 'Administrar centros' },
        { href: '/admin/materias', label: 'Gestión de Materias', icon: BookOpen, color: 'bg-purple-500', description: 'Configurar materias' }
      )
    }
    
    if (rol === 'estudiante') {
      accesos.push(
        { href: '/dashboard/mis-notas', label: 'Mis Notas', icon: TrendingUp, color: 'bg-blue-500', description: 'Ver calificaciones' },
        { href: '/dashboard/asistencia', label: 'Mi Asistencia', icon: ClipboardCheck, color: 'bg-green-500', description: 'Ver asistencia' }
      )
    }
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      {/* Tarjetas de KPIs (solo para roles que no son estudiante) */}
      {rol !== 'estudiante' && (
            ...
            <div className="space-y-3">
              {rol === 'coordinador' && (
                <>
                  ...
                  <p className="text-sm">📊 Visualiza las estadísticas académicas del centro</p>
                </>
              )}
              {rol === 'docente' && (
                <>
              ...
              {rol === 'admin' && (
                <>
                  <p className="text-sm">🌐 Administra todos los centros educativos</p>
                  <p className="text-sm">📚 Gestiona las materias del sistema</p>
                </>
              )}
            </div>
          </CardContent>
