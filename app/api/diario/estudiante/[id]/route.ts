...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  Object.entries(porPeriodo).forEach(([p, regs]) => {
    ...
    resumen[p] = {
      ...
      estrellas: regs.reduce((a, b) => a + (b.participacion || 0), 0),
      tareas: regs.filter(r => r.tarea).length,
      puntosExtra: regs.reduce((a, b) => a + (b.puntosExtra || 0), 0),
    }
  })

  return NextResponse.json({ registros: porPeriodo, resumen })
}