...
export default function DiarioPage() {
  ...
  const getRegistro = (estudianteId: string) => registros.find(r => r.estudianteId === estudianteId) || {}
  ...
  const guardar = async () => {
    const data = estudiantes.map(e => ({
      ...
      observacion: (document.getElementById(`obs-${e._id}`) as HTMLSelectElement)?.value || '',
          ...
          {isMobile ? (
            <div className="space-y-3">
              {estudiantes.map(e => {
                ...
                return (
                  <div key={e._id} className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-3">
                    <p className="font-medium text-gray-900 dark:text-white text-sm mb-2">{e.nombre}</p>
                    <div className="grid grid-cols-2 gap-2 text-xs">
                      <div>
                        <label className="text-gray-500">⭐ Participación (0-5)</label>
                        <input id={`part-${e._id}`} type="number" min="0" max="5" defaultValue={reg.participacion ?? 0}
                          className="w-full px-2 py-1 border rounded dark:bg-slate-700 dark:text-white mt-0.5" />
                      </div>
                      <div>
                        <label className="text-gray-500">🎁 Extra</label>
                        <input id={`extra-${e._id}`} type="number" min="0" max="10" defaultValue={reg.puntosExtra ?? 0}
                          className="w-full px-2 py-1 border rounded dark:bg-slate-700 dark:text-white mt-0.5" />
                      </div>
                      <div>
                        <label className="text-gray-500">📝 Tarea</label>
                        <input id={`tarea-${e._id}`} type="checkbox" defaultChecked={reg.tarea || false} className="ml-2 mt-2 w-4 h-4" />
                      </div>
                      <div>
                        <label className="text-gray-500">💬 Obs</label>
                        <select id={`obs-${e._id}`} defaultValue={reg.observacion || ''}
                          className="w-full px-2 py-1 border rounded dark:bg-slate-700 dark:text-white mt-0.5 text-xs">
                  ...
                  {estudiantes.map(e => {
                    ...
                    return (
                      <tr key={e._id}>
                        ...
                        <td className="px-3 py-3 text-center">
                          <input id={`tarea-${e._id}`} type="checkbox" defaultChecked={reg.tarea || false} className="w-5 h-5" />
                        </td>
                        <td className="px-3 py-3 text-center">
                          <input id={`extra-${e._id}`} type="number" min="0" max="10" defaultValue={reg.puntosExtra ?? 0}
                            className="w-16 px-2 py-1 border rounded text-center dark:bg-slate-700 dark:text-white" />
