    ...
    <div className="max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
          {/* Switch escolar */}
          <label className="flex items-center gap-3 cursor-pointer mb-6 p-4 bg-gray-50 dark:bg-slate-700/30 rounded-lg">
            <input
              type="checkbox"
              checked={esEscolar}
              onChange={(e) => { setEsEscolar(e.target.checked); setForm({ ...form, nivel: '', ciclo: '', grado: '' }) }}
              className="w-4 h-4 rounded"
            />
            <div>
              <p className="text-sm font-medium text-gray-900 dark:text-white">Estudiante escolar</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">Asignar nivel, ciclo y grado</p>
            </div>
          </label>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div><label className={labelClass}>RNE (opcional)</label><input type="text" value={form.rne} onChange={(e) => setForm({ ...form, rne: e.target.value })} className={inputClass} /></div>
          </div>
        </div>
